x = Tam_Populacao;                          % x Recebe o Tamanho da População definida pelo usuário
Tam_Vetor_Inic = length(Matriz_Custo(1,:)); % Tam_Vetor_Inic recebe o tamanho do indivíduo
Func_Obj = 0;

% Função para obter o valor das Funções-objetivo
...
if Var_Controle2 == 1
   Melhor_FuncObj = Func_Obj(1); % Define um idivíduo qualquer para ser o Melhor indivíduo 
...
% Função q faz comparações com o vetor 'Func_Obj' para obter o Melhor e o Pior Indivíduo
while x > 0,  
      if Func_Obj(x)< Melhor_FuncObj
         Melhor_FuncObj = Func_Obj(x);         
         Melhor_Individuo = Populacao_Atual(x,:);
         Melhor_Individuo_Global = Melhor_Individuo; %***** Elitismo (2ª PArte) *****%
...
%Função que obtem o fitness de cada indivíduo e coloca no vetor 'fitness'
while x > 0,  
      Var(x) = Func_Obj(1,x);
      fitness(1,x) = 1/Var(x); % Obtém o fitness dos vetores
      x = x-1;
end
...
% Calcula o Indivíduo de Melhor fitness
while x > 0,  
...
% Calcula a Média do fitness
Soma_fitness  = sum(fitness(1,:));
Media_fitness = Soma_fitness/Quant_Ind_Inic;


% Vetores de para armazenar os Dados ao Longo das Gerações
